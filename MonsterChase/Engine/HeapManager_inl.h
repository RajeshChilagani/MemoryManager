...
namespace HeapManagerProxy
{
	...
	inline bool HeapManager::_Contains(void * i_ptr)
	{
	...
	{
		assert(i_pManager);
		return const_cast<HeapManager *>(i_pManager)->_Contains(i_ptr);
	}
	inline bool IsAllocated(const HeapManager * i_pManager, void * i_ptr)
	{
		assert(i_pManager);
		return const_cast<HeapManager *>(i_pManager)->_IsAllocated(i_ptr);
	}
	inline size_t GetLargestFreeBlock(const HeapManager * i_pManager)
	{
		assert(i_pManager);
		return const_cast<HeapManager *>(i_pManager)->getLargestFreeBlock();
	}
	inline size_t GetTotalFreeMemory(const HeapManager * i_pManager)
	{
		assert(i_pManager);
		return const_cast<HeapManager *>(i_pManager)->getTotalFreeMemory();
	}
	inline void ShowFreeBlocks(const HeapManager * i_pManager)
	{
		assert(i_pManager);
		const_cast<HeapManager *>(i_pManager)->_ShowFreeBlocks();
	}
	inline void ShowOutstandingAllocations(const HeapManager * i_pManager)
	{
		assert(i_pManager);
		const_cast<HeapManager *>(i_pManager)->_ShowOutstandingAllocations();
	}
}
